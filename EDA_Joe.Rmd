...
#Rotation Rating
            rotation_rating = rotation/max(significance$rotation, na.rm = TRUE),
#Rating Rating
            rating_rating = ratings/max(significance$ratings, na.rm = TRUE),
#Debut Rating
            date = parse_date(date),
            debut = parse_date(debut),
            since_debut = ifelse(as.numeric(date - debut) > 0, as.numeric(date - debut), 0),
            since_debut_rating = since_debut/max(significance$since_debut, na.rm = TRUE)
#Cover Bonus
#Notes Bonus
)
...
significance <-  significance  %>% 
    mutate(tease_dummy = as.numeric(grepl('tease|quote', notes)))
...
significance %>% 
    ...
    geom_smooth(aes(teases, ratings)) +
...
significance %>% 
    ...
    summarise(number_of_shows_per_tour = n(), 
              ...
              rating = mean(ratings)) %>% 
    ggplot() +
    geom_point(aes(id_tour, rating, color = number_of_shows_per_tour)) +
    scale_color_gradientn(colors = c('blue', 'red')) +
    geom_smooth(aes(id_tour, rating)) +
    ggtitle("How has the percieved quality of shows changes over time - by tour") +
    xlab("Chronological numbering of tour") +
    ylab("Rating from Phish.Net")
