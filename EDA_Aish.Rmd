...
ggplot(phanalytix) + 
    ...
    labs(x="Song ID", y = "Duration of Son") +
    theme(panel.background = element_blank())
...
ggplot(orig_phish) +
    aes(artist == "Phish") +
    geom_bar() + 
    ggtitle ("36% of Total Songs Performed are Originally Written by Phish") + 
...
Figure 7 shows that, out of all the songs performed by Phish, only 36% are originally written by the band.  
...
ggplot(covers_phish) +
    aes(cover_dummy == "1") +
    geom_bar () +
    ggtitle ("63% of Total Songs Performed are Covers") +
    geom_text(stat = 'count',aes(label=..count..), vjust=1.25, color="white") +
...
Figure 8 shows, out of all the songs performed by Phish, 63% are covers.  This makes sense, because, from the earlier plot, we know that 36% are originally written by Phish, so most of the other songs have to be covers.  
...
Figure 10 is a bar graph of the number of debut songs performed by Phish over the years.  The shape of the graph is an inverse shape, with the highest numbers of debuts at 20 per year, during 1990 and 1997.  The lowest number of debuts is 1, during 2000.  
...
base_map <- ggplot() + 
            ...
            xlab("") + 
            ylab("") +
            labs(title = "Debut Songs' Performance Concentrated in North East and Western US")
...
Figure 11 is a basic map of the world which shows the locations of the debuts of each song.  The red points indicate these observations.  We can see that most of the debuts occurred in the Northeast and Western parts of the US. 
